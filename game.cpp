...
game::game()
{
    ...
    arial.loadFromFile("/System/Library/Fonts/Supplemental/Arial Unicode.ttf");
    
    text_state.setFont(arial);
    text_state.setPosition(constants::window_width/2.f-70.f, constants::window_height/2.f-100.f);
    text_state.setCharacterSize(30);
    text_state.setFillColor(sf::Color::White);
    text_state.setString("Paused");

    text_lives.setFont(arial);
    text_lives.setPosition(constants::window_width/2.f-65.f, constants::window_height/2.f-60.f);
    text_lives.setCharacterSize(30);
    text_lives.setFillColor(sf::Color::White);
    text_lives.setString("Lives Left: "+std::to_string(lives));
}
...
void game::reset()
{
    ...
    manager.create<ball>(constants::window_width/2.0f,constants::window_height/2.f);
    manager.create<paddle>(constants::window_width/2.f,
                           constants::window_height-constants::paddle_height);

    for (int i=0; i<constants::brick_colums; ++i)
    {
...
void game::run()
{
    ...
    while(game_window.isOpen())
    {
        ...
        //check for events
        //if user pressed escape o clicked on close, we close the window, this terminates loop
        sf::Event event;
